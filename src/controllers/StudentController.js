...
async function validateCreateStudent(args){
    ...
    try {
        ...
        if(student){
            ...
            const validateStudent = await StudentDispatcher.getStudent(student.student_email);

            //Check if student returns from our database and if not create a new student
            if(validateStudent.data.Count < 1) {
                console.error(`Student doesn't exist in our database. Student needs to be created`);
                ...
                if(createStudent.data.student_id){
                    console.log(`Student has been created with Student_id: `, createStudent.data.student_id);
                    studentCheck = true;
                } else {
